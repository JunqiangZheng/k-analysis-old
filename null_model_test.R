...
for (name_red in listfiles)
{
  ...
  valnodfs <- valnodfs[!is.nan(valnodfs)]
  media_nodf <- mean(valnodfs)
  sd_nodf <- sd(valnodfs)
  z_nodf <- (obsnodf-media_nodf)/sd_nodf
  if (!analizatodo){
    plot(density(valnodfs), xlim=c(min(obsnodf, min(valnodfs)), max(obsnodf, max(valnodfs))), 
  ...
  valmodularity <- valmodularity[!is.nan(valmodularity)]
  media_modularity <- mean(valmodularity)
  sd_modularity <- sd(valmodularity)
  z_modularity <- (obsmodularity-media_modularity)/sd_modularity
  if (!analizatodo){
    plot(density(valmodularity), xlim=c(min(obsmodularity, min(valmodularity)), max(obsmodularity, max(valmodularity))), 
  ...
  valradius <- valradius[!is.nan(valradius)]
  media_radius <- mean(valradius)
  sd_radius <- sd(valradius)
  z_radius <- (obsradius-media_radius)/sd_radius
  if (!analizatodo){
    plot(density(valradius), xlim=c(min(obsradius, min(valradius)), max(obsradius, max(valradius))), 
  ...
  valspecradius <- valspecradius[!is.nan(valspecradius)]
  media_specradius <- mean(valspecradius)
  sd_specradius <- sd(valspecradius)
  z_specradius <- (obsspecradius-media_specradius)/sd_specradius
  if (!analizatodo){
    plot(density(valspecradius), xlim=c(min(obsspecradius, min(valspecradius)), max(obsspecradius, max(valspecradius))), 
...
#   }
  ...
  if (analizatodo | write_results){
    ...
    save(auxdfnulls, file=paste0('resultsnulls/',strsplit(name_red,".csv")[[1]],'_auxdfnulls_',paste(tipos_de_red, collapse = '_'),'_cycles_',intentos,'_method_',metodo,'.RData'), compress=TRUE)
    save(dfindivs, file=paste0('resultsnulls/',strsplit(name_red,".csv")[[1]],'_dfindivs_',paste(tipos_de_red, collapse = '_'),'_cycles_',intentos,'_method_',metodo,'.RData'), compress=TRUE) 
  }
}
