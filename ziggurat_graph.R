                            ...
                            pbackground = "yes", joinchars = "\n", tspline = "no", is_guild_a = TRUE)
{
  ...
  for (i in list_spec)
  {
    ...
    if (length(conn_species)>0)
    {
      ...
      v<- draw_tail(p,little_tail,zgg$lado,color_guild[1],
...
draw_weird_chains <- function(grafo, df_chains, paintsidex)
{
  p <- grafo
                         ...
                         inverse="no", is_guild_a =TRUE)
{
  ...
  if (nrow(fat_tail)>0)
  {
    ...
    v<- draw_tail(p,fat_tail,zgg$lado,color_guild,gen_sq_label(fat_tail$orph),
...
handle_weirds <- function(p,weirds_a,weirds_b,lado,gap)
{
  ...
  if  (( ( nrow(weirds_a)+nrow(weirds_b) )>0)) {
    ...
    p <- draw_weird_chains(p, zgg$df_chains, zgg$lado)
  }
  calc_vals <- list("p" = p, "df_chains" = zgg$df_chains) 
  return(calc_vals)
}

write_annotations <- function(p)
{
  if (zgg$show_title)
...
draw_inner_orphans <- function(p)
{
  if (zgg$kcoremax >2)
  {
    for (kc in seq(from = zgg$kcoremax-1, to = 2))
    {
      ...
      if (exists("df_orph_a", envir = zgg)){
        w <- draw_innercores_tails(p,kc,zgg$list_dfs_b,zgg$df_orph_a,zgg$color_guild_a, inverse="no")
        p <- w["p"][[1]]
    ...
    zgg$kcore2tail_vertical_separation <- zgg$kcore2tail_vertical_separation + 2
    if (exists("df_orph_a", envir = zgg)){
      w <- draw_innercores_tails(p,2,zgg$list_dfs_b,zgg$df_orph_a,zgg$color_guild_a, inverse="yes")
      p <- w["p"][[1]]
...
draw_all_ziggurats <- function(p)
{
  ...
  calc_vals <- list("p" = p, "posic_zig" = zgg$posic_zig, "list_dfs_a" = zgg$list_dfs_a, "list_dfs_b" = zgg$list_dfs_b,
...
draw_maxcore <- function()
{
  ...
  p <- kcoremax_label_display(p,kcoremaxlabel_angle,zgg$list_dfs_a[[zgg$kcoremax]],labelszig,zgg$lsize_kcoremax)
...
draw_coremax_tails <- function(p)
{
  ...
  if ((exists("long_tail_a")) & (zgg$kcoremax > 2))
  {
    ...
    v<-  draw_edge_tails(p,point_x,(point_y+zgg$height_y*zgg$aspect_ratio),zgg$kcoremax,long_tail_a,zgg$list_dfs_b,zgg$color_guild_a, inverse = "yes", 
