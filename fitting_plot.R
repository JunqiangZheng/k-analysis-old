...
source("network-kanalysis.R")
  ...
  s <- ggplot(aes(x = NODF, y = resid(lm(log(MeanKdistance) ~ NODF))), data = resultdf )+
    geom_hline(yintercept = 0, size = 1, color = "orchid")+
    geom_point(aes(size=3, colour = factor(Type)), alpha = 0.6)+
    scale_fill_manual(values=c("chocolate3", "cyan4"),name="Type") +
    scale_colour_manual(values=c("chocolate3", "cyan4")) +
  guides(colour = guide_legend(override.aes = list(shape = 20, size = 8)),
         size = FALSE)+
...
ppi <- 600
png("corr_figs.png", width=(11*ppi), height=4*ppi, res=ppi)
